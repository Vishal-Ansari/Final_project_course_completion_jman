        ...
        <div className="bg-gray-50 flex justify-center items-center min-h-screen">
            <div className="max-w-xl border border-gray-300 mx-auto bg-white p-10 shadow-md rounded-lg">
                ...
                <div className="mb-4">
                    ...
                    >
                        <span>{learningPath.selectedCourses.length > 0 ? 
                            courses.filter(course => learningPath.selectedCourses.includes(course._id)).map(course => course.title).join(', ') : 
                            "Select Courses"}</span>
                            ...
                            {courses.map(course => (
                                <div key={course._id} className="flex items-center mb-2">
                                    <input
                                        ...
                                        type="checkbox"
                                        value={course._id}
                                        checked={learningPath.selectedCourses.includes(course._id)}
                                        onChange={(e) => {
                                        ...
                                        className="w-4 h-4"
                                    />
                                    <label htmlFor={course._id} className="ml-2 text-gray-600">{course.title}</label>
                                </div>
                            ))}
                        </div>
                    )}
